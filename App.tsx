...
const App: React.FC = () => {
  ...
  const [currentQuote, setCurrentQuote] = useState(() => FOOTER_QUOTES[Math.floor(Math.random() * FOOTER_QUOTES.length)]);
  ...
  const [analytics, setAnalytics] = usePersistentState<AnalyticsData>('app-analytics', {
    ...
    weeklyProgress: [
      { name: 'Sun', points: 0 }, { name: 'Mon', points: 0 },
      { name: 'Tue', points: 0 }, { name: 'Wed', points: 0 },
      { name: 'Thu', points: 0 }, { name: 'Fri', points: 0 },
      { name: 'Sat', points: 0 },
    ],
  ...
  useEffect(() => {
    const quoteInterval = setInterval(() => {
        setCurrentQuote(prevQuote => {
            let newQuote;
            // Ensure we don't pick the same quote twice in a row if there's more than one option.
            if (FOOTER_QUOTES.length > 1) {
              do {
                newQuote = FOOTER_QUOTES[Math.floor(Math.random() * FOOTER_QUOTES.length)];
              } while (newQuote.quote === prevQuote.quote);
            } else {
              newQuote = FOOTER_QUOTES[0];
            }
            return newQuote;
        });
    }, 180000); // 3 minutes
  ...
  const contextValue = useMemo(() => ({
    ...
    isChatbotOpen,
    setIsChatbotOpen,
  }), [currentPage, setCurrentPage, theme, toggleTheme, points, analytics, codeToLoad, user, setAnalytics, setPoints, setCodeToLoad, setUser, isChatbotOpen]);
    ...
    <AppContext.Provider value={contextValue}>
      ...
      <div className={`bg-light-bg dark:bg-dark-bg text-light-text-primary dark:text-dark-text-primary min-h-screen flex flex-col md:flex-row transition-colors duration-300`}>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <Header />
          <main className="flex-1 p-3 sm:p-4 md:p-6 lg:p-8 overflow-y-auto flex flex-col pb-16 md:pb-6">
            ...
            <div style={{ display: currentPage === 'Get In Touch' ? 'flex' : 'none' }} className="flex-grow flex flex-col">
             ...
             <footer className="md:hidden text-center py-4 text-light-text-secondary dark:text-dark-text-secondary text-sm px-4 mt-auto">
