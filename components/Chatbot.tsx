...
import type { ChatMessage, AppContextType } from '../types';
import { AppContext } from '../context/AppContext';

const Chatbot: React.FC = () => {
  const { isChatbotOpen: isOpen, setIsChatbotOpen: setIsOpen } = useContext(AppContext) as AppContextType;
  ...
  const sendMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      if (firstChunk) { // Handle cases with no response text
        setIsLoading(false);
      }
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          ...
          {isLoading && (
              <div className="flex items-end gap-2 justify-start">
                  ...
                  <div className="max-w-[85%] p-3 rounded-lg bg-light-bg dark:bg-dark-hover text-light-text-primary dark:text-dark-text-primary flex items-center space-x-1.5">
                      <div className="dot1 w-2 h-2 bg-gray-500 rounded-full"></div>
                      <div className="dot2 w-2 h-2 bg-gray-500 rounded-full"></div>
                      <div className="dot3 w-2 h-2 bg-gray-500 rounded-full"></div>
                  </div>
              </div>
          )}
           {showSuggestions && (
